...
class IBKRService(BaseBroker):
    ...
    async def connect(self) -> None:
        """Alias to initialize to satisfy the broker interface contract."""
    ...
    async def place_order(
        ...
        """Place a trading order and return the execution metadata."""
        if not self.is_connected():
            logger.error("Not connected to IBKR")
            return None
        ...
        try:
            ...
            if stop_loss or take_profit:
                ...
                return {"parent_order_id": parent_order_id}
            else:
